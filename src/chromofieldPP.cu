...
__global__ void kernel_ChromoFieldPP(ChromoFieldArgPP arg){
...
__global__ void kernel_ChromoFieldMidFluxTubePP(ChromoFieldArgPP arg){
...
class ChromoFieldPP: Tunable{
private:
   ChromoFieldArgPP arg;
   ...
   void apply(const cudaStream_t &stream){
	...
	field->Clear();
	if(chargeplane) kernel_ChromoFieldPP<<<tp.grid, tp.block, tp.shared_bytes, stream>>>(arg);
	else kernel_ChromoFieldMidFluxTubePP<<<tp.grid, tp.block, tp.shared_bytes, stream>>>(arg);
}

public:
   ChromoFieldPP(Array<complexd> *ploop, Array<complexd> *plaqfield, Array<complexd> *chromofield, int radius, int nx, int ny): chromofield(chromofield){
	size = SpatialVolume();
	timesec = 0.0;
	arg.nx = nx;
	arg.ny = ny;
   ...
   void stat(){	cout << "ChromoFieldPP:  " <<  get_time() << " s\t"  << bandwidth() << " GB/s\t" << flops() << " GFlops"  << endl;}
...
template<bool chargeplane>
void CalcChromoFieldPP(Array<complexd> *ploop, Array<complexd> *plaqfield, Array<complexd> *field, int radius, int nx, int ny){
	Timer mtime;
	mtime.start(); 
	ChromoFieldPP<chargeplane> cfield(ploop, plaqfield, field, radius, nx, ny);
	cfield.Run();
	cudaDevSync( );
	mtime.stop();
...
void CalcChromoFieldPP(Array<complexd> *ploop, Array<complexd> *plaqfield, Array<complexd> *field, int radius, int nx, int ny, bool chargeplane){
	if(Dirs() < 4){
		cout << "Only implemented for 4D lattice..." << endl;
		Finalize(1);
	}
	if(chargeplane) CalcChromoFieldPP<true>(ploop, plaqfield, field, radius, nx, ny);
	else CalcChromoFieldPP<false>(ploop, plaqfield, field, radius, nx, ny);
}
